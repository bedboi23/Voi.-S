...
def create_now_playing_embed(player, current_pos, total_duration):
    ...
    # Time format like reference (0:00 / 2:32)
    current_time = format_time(current_pos)
    total_time = format_time(total_duration)
    
    # Embed matching EXACT reference layout
...
`{bar}` **{current_time} / {total_time}**
"
    
    # Thumbnail above controls (like reference)
    if player.thumbnail:
...
async def play(interaction: discord.Interaction, query: str):
    ...
    try:
        ...
        now_playing[interaction.guild.id] = {'message': message, 'player': player, 'start_time': time.time()}
        ...
        await interaction.followup.send(f"❌ Error playing: {str(e)}", ephemeral=True)
...
async def stop(interaction: discord.Interaction):
    ...
    if vc:
        ...
        if interaction.guild.id in now_playing:
            try:
                await now_playing[interaction.guild.id]['message'].delete()
            except:
                pass
